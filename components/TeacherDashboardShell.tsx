...
const TABS: TabConfig[] = [
  { id: 'overview', label: 'Overview', subtitle: 'Quick snapshot' },
  { id: 'students', label: 'Students', subtitle: 'Classroom management' },
  { id: 'homework', label: 'Homework', subtitle: 'Assignments and review' },
  { id: 'teaching', label: 'Teaching', subtitle: 'Lessons and materials' },
  { id: 'grades', label: 'Grades', subtitle: 'Score tracking' },
  { id: 'communication', label: 'Communication', subtitle: 'Announcements and live classes' }
];
  ...
  return (
    <div className="space-y-8">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <OverviewCard label="Students" value={studentCount} helper={`${overview.activeStudents} active learners`} />
        <OverviewCard label="Homework" value={assignments.length} helper={`${overview.pendingReview} total submissions to review`} />
        <OverviewCard label="Live Classes" value={overview.upcomingClasses} helper="Upcoming sessions on the calendar" />
        <OverviewCard label="Announcements" value={announcements.length} helper="Recent updates for students and parents" />
      </div>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-200/50">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-600">Jump between sections instead of scrolling through one long page.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setActiveTab('teaching')} className="rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700">
              Add lesson
            </button>
            <button type="button" onClick={() => setActiveTab('homework')} className="rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
              Create assignment
            </button>
            <button type="button" onClick={() => setActiveTab('communication')} className="rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
              Post announcement
            </button>
            <button type="button" onClick={() => setActiveTab('grades')} className="rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
              Add grade
            </button>
          </div>
        </div>
      ...
      {activeTab === 'teaching' && (
        <div className="space-y-6">
          <div className="grid gap-6 xl:grid-cols-2">
            ...
            <SectionCard title="Lesson Library" description="Edit or remove lessons and keep the library organized by level.">
      ...
      {activeTab === 'communication' && (
        <div className="grid gap-6 xl:grid-cols-2">
          ...
          <SectionCard title="Live Classes" description="Schedule Zoom or Google Meet sessions and keep your calendar current.">
