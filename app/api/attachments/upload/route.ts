...
export async function POST(request: Request) {
  ...
  if (!isTeacherUser(profile, user)) {
    return NextResponse.json({ error: 'Only teachers can upload attachments.' }, { status: 403 });
  }
  ...
  if (!ALLOWED_EXTENSIONS.has(extension)) {
    return NextResponse.json(
      { error: 'Unsupported file type. Use PDF, Word, Excel, PowerPoint, text, or image files.' },
      { status: 400 }
    );
  }
